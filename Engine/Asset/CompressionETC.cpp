...
namespace etc
{
	...
	{
		wassume((key_ofs >= 0) && (key_ofs < sizeof(T)));
		wassume((key_size >= 1) && (key_size <= 4));
...
namespace etc
{
	...
	TexCompressionETC1::Results& TexCompressionETC1::Results::operator=(TexCompressionETC1::Results const & rhs)
	{
		if (this != &rhs)
		{
			...
			error_ = rhs.error_;
			wassume(selectors_.size() == rhs.selectors_.size());
	...
	uint64 TexCompressionETC1::EncodeETC1BlockInternal(ETC1Block& dst_block, ARGBColor32 const * argb, TexCompressionMethod method)
	{
		...
		wassume(best_use_color4 || ((std::min({ dr, dg, db }) >= -4) && (std::min({ dr, dg, db }) <= 3)));
...
#ifndef NDEBUG
		uint32 const limit = diff ? 32 : 16;
		wassume((diff < 2) && (inten < 8) && (selector < 4) && (packed_c < limit));
#endif
		int c;
		if (diff)
		{
	...
	{
		wassume(etc1_inverse_lookup_[0][255]);
			...
			for (int delta = -delta_range; (delta <= delta_range) && (best_err != 0); ++ delta)
			{
				...
				do
				{
...
#ifndef NDEBUG
					{
						...
						uint32 p0 = (x >> 8) & 255;
						wassume(ETC1DecodeValue(diff, inten, selector, p0) == static_cast<uint32>(c_plus_delta));
					}
#endif
	...
	void TexCompressionETC1::InitSolver(Params const & params, Results& result)
	{
		...
		wassume(8 == params.num_src_pixels_);
	...
	bool TexCompressionETC1::Solve()
	{
...
#ifndef NDEBUG
		{
			...
			wassume(actual_err == best_solution_.error_);
		}
#endif      
