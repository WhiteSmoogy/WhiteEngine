...
template< typename T >
bool LUPFactorize( T* wrestrict A, uint32* wrestrict Pivot, uint32 Size, T Epsilon )
{
	...
	for( uint32 i = 0; i < Size; i++ )
	{
		...
		// std::swap rows pivoting MaxValue to the diagonal
		if( MaxIndex != i )
		{
...
template< typename T >
void LUPSolve( const T* wrestrict LU, const uint32* wrestrict Pivot, uint32 Size, const T* wrestrict b, T* wrestrict x )
{
	for( uint32 i = 0; i < Size; i++ )
	{
		x[i] = b[ Pivot[i] ];
...
template< typename T >
bool LUPSolveIterate( const T* wrestrict A, const T* wrestrict LU, const uint32* wrestrict Pivot, uint32 Size, const T* wrestrict b, T* wrestrict x )
{
	T* Residual = (T*)lalloca( 2 * Size * sizeof(T) );
	...
	for( uint32 k = 0; k < 4; k++ )
	{
		...
		if( MeanSquaredError < wm::KindaEpsilon )
		{
			bCloseEnough = true;
			break;
		}
...
FQuadric::FQuadric( const wm::float3& fp0, const wm::float3& fp1, const wm::float3& fp2 )
{
	const QVec3 p0( fp0 );
	const QVec3 p1( fp1 );
	const QVec3 p2( fp2 );
...
FQuadric::FQuadric( const wm::float3& fp0, const wm::float3& fp1, const wm::float3& faceNormal, const float edgeWeight )
{
...
float FQuadric::Evaluate( const wm::float3& Point ) const
{
	// Q(v) = vt*A*v + 2*bt*v + c
...
FQuadricAttr::FQuadricAttr(
	const wm::float3& fp0, const wm::float3& fp1, const wm::float3& fp2,
	...
	const float* AttributeWeights, uint32 NumAttributes )
{
	...
	QVec3* wrestrict   g = (QVec3*)( this + 1 );
...
void FQuadricAttr::Rebase(
	const wm::float3& wrestrict Point,
	const float* wrestrict Attribute,
	const float* wrestrict AttributeWeights,
...
void FQuadricAttr::Add(
	const FQuadricAttr& wrestrict q,
	const wm::float3& wrestrict Point,
	const float* wrestrict Attribute,
	const float* wrestrict AttributeWeights,
	uint32 NumAttributes )
	...
	uint32 NumAttributes )
{
	...
	QVec3* wrestrict   qg = (QVec3*)( &q + 1 );
	QScalar* wrestrict qd = (QScalar*)( qg + NumAttributes );
...
float FQuadricAttr::Evaluate( const wm::float3& Point, const float* wrestrict Attributes, const float* wrestrict AttributeWeights, uint32 NumAttributes ) const
{
	// Q(v) = vt*A*v + 2*bt*v + c
	...
	QVec3* wrestrict   g = (QVec3*)( this + 1 );
	QScalar* wrestrict d = (QScalar*)( g + NumAttributes );
	QScalar* wrestrict s = (QScalar*)lalloca( NumAttributes * sizeof( QScalar ) );
...
float FQuadricAttr::CalcAttributesAndEvaluate( const wm::float3& wrestrict Point, float* wrestrict Attributes, const float* wrestrict AttributeWeights, uint32 NumAttributes ) const
{
	// Q(v) = vt*A*v + 2*bt*v + c
...
bool FQuadricAttrOptimizer::Optimize( wm::float3& Position ) const
{
	// A * v = -b
...
bool FQuadricAttrOptimizer::OptimizeVolume( wm::float3& Position ) const
{
	// A * v = -b
...
bool FQuadricAttrOptimizer::OptimizeLinear( const wm::float3& Position0, const wm::float3& Position1, wm::float3& Position ) const
{
