...
template< typename T >
TVec3<T> operator*( float Scalar, const TVec3<T>& v )
{
...
public:
	FEdgeQuadric() {}
	FEdgeQuadric( const wm::float3& p0, const wm::float3& p1, const wm::float3& FaceNormal, const float Weight );
...
inline FEdgeQuadric::FEdgeQuadric( const wm::float3& fp0, const wm::float3& fp1, const wm::float3& FaceNormal, const float Weight )
{
	if( !wm::is_normalize(FaceNormal))
	{
		Zero();
		return;
	}
	...
	if( Length < (QScalar)wm::SmallNumber)
	{
		Zero();
		return;
	}
...
public:
	...
	FQuadric( const wm::float3& p0, const wm::float3& p1, const wm::float3& p2 );
	// Quadric for boundary edge
	FQuadric( const wm::float3& p0, const wm::float3& p1, const wm::float3& faceNormal, const float edgeWeight );
	
	void		Zero();

	FQuadric&	operator+=( const FQuadric& q );

	void		Add( const FEdgeQuadric& q, const wm::float3& Point );
	
	// Evaluate error at point
	float		Evaluate( const wm::float3& p ) const;
...
inline void FQuadric::Add( const FEdgeQuadric& wrestrict EdgeQuadric, const wm::float3& Point )
{
...
public:
				FQuadricAttr() {}
				FQuadricAttr(
					const wm::float3& p0, const wm::float3& p1, const wm::float3& p2,
					const float* a0, const float* a1, const float* a2,
					const float* AttributeWeights, uint32 NumAttributes
					);

	void		Rebase( const wm::float3& Point, const float* Attributes, const float* AttributeWeights, uint32 NumAttributes );
	void		Add( const FQuadricAttr& q, const wm::float3& Point, const float* Attribute, const float* AttributeWeights, uint32 NumAttributes );
	...
	float		Evaluate( const wm::float3& Point, const float* Attributes, const float* AttributeWeights, uint32 NumAttributes ) const;
	
	// Calculate attributes for point and evaluate error
	float		CalcAttributesAndEvaluate( const wm::float3& Point, float* Attributes, const float* AttributeWeights, uint32 NumAttributes ) const;
...
public:
	...
	void		Rebase( const wm::float3& Point, const float* Attributes, const float* AttributeWeights );
	void		Add( const TQuadricAttr< NumAttributes >& q, const wm::float3& Point, const float* Attribute, const float* AttributeWeights );
	...
	// Evaluate error at point with attributes and weights
	float		Evaluate( const wm::float3& Point, const float* Attributes, const float* AttributeWeights ) const;
	
	// Calculate attributes for point and evaluate error
	float		CalcAttributesAndEvaluate( const wm::float3& Point, float* Attributes, const float* AttributeWeights ) const;
...
inline void TQuadricAttr< NumAttributes >::Rebase(
	const wm::float3& Point, const float* Attribute,
	const float* AttributeWeights )
{
...
template< uint32 NumAttributes >
inline TQuadricAttr< NumAttributes >& TQuadricAttr< NumAttributes >::operator+=( const TQuadricAttr< NumAttributes >& wrestrict q )
{
	nxx += q.nxx;
	nyy += q.nyy;
	nzz += q.nzz;
...
template< uint32 NumAttributes >
inline float TQuadricAttr< NumAttributes >::CalcAttributesAndEvaluate( const wm::float3& Point, float* wrestrict Attributes, const float* wrestrict AttributeWeights ) const
{
...
public:
	...
	bool		Optimize( wm::float3& Position ) const;
	bool		OptimizeVolume( wm::float3& Position ) const;
	bool		OptimizeLinear( const wm::float3& Position0, const wm::float3& Position1, wm::float3& Position ) const;
...
inline void FQuadricAttrOptimizer::AddQuadric( const FQuadric& wrestrict q )
{
	nxx += q.nxx;
	nyy += q.nyy;
	nzz += q.nzz;
...
inline void FQuadricAttrOptimizer::AddQuadric( const FQuadricAttr& wrestrict q, uint32 NumAttributes )
{
	if( q.a < (QScalar)wm::SmallNumber )
		return;
	...
	QVec3* wrestrict   g = (QVec3*)( &q + 1 );
	QScalar* wrestrict d = (QScalar*)( g + NumAttributes );
