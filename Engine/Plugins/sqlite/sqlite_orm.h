...
#define SQLITE_ORM_EXCEPTED_SUPPORTED 1

#include <WBase/folly/Expected.h>
#undef SQLITE_ORM_OPTIONAL_SUPPORTED
#define SQLITE_ORM_OPTIONAL_SUPPORTED 0

namespace sqlite_orm
{
    using sqlite_error = std::system_error;
}

namespace std
{
   template<class CharT>
   struct formatter<sqlite_orm::sqlite_error,CharT> : std::formatter<const char*,CharT>
   {
...
namespace sqlite_orm {
    ...
    namespace tuple_helper {
        ...
        template<size_t N, class... Args>
        struct iterator_expected {

            template<class L>
            folly::Expected<folly::Unit, sqlite_error> operator()(const std::tuple<Args...>& t, const L& l, bool reverse = true) {
                if (reverse) {
                    auto expected = l(std::get<N>(t));
                    if(expected)
                        return iterator_expected<N - 1, Args...>()(t, l, reverse);
                    return expected;
                }
                else {
                    auto expected = iterator_expected<N - 1, Args...>()(t, l, reverse);
                    if(expected)
                        return l(std::get<N>(t));
                    return expected;
                }
            }
        };
        ...
        template<class... Args>
        struct iterator_expected<0, Args...> {

            template<class L>
            folly::Expected<folly::Unit, sqlite_error> operator()(const std::tuple<Args...>& t, const L& l, bool /*reverse*/ = true) {
                return l(std::get<0>(t));
            }
        };

        template<size_t N>
        struct iterator_expected<N> {

            template<class L>
            folly::Expected<folly::Unit, sqlite_error> operator()(const std::tuple<>&, const L&, bool /*reverse*/ = true) {
                return folly::makeExpected<sqlite_error>(folly::unit);
            }
        };
    ...
    namespace internal {
        ...
        template<class L, class... Args>
        folly::Expected<folly::Unit,sqlite_error> iterate_tuple_expected(const std::tuple<Args...>& t, const L& l) {
            using tuple_type = std::tuple<Args...>;
            return tuple_helper::iterator_expected<std::tuple_size<tuple_type>::value - 1, Args...>()(t, l, false);
        }
...
namespace sqlite_orm {
    ...
    namespace internal {
        ...
        struct table_t : table_base {
             ...
             */
            template<class L>
            folly::Expected<folly::Unit, sqlite_error> for_each_column(const L &l) const {
                return iterate_tuple_expected(this->columns, [&l](auto &column) {
                    using column_type = typename std::decay<decltype(column)>::type;

                    if constexpr (std::is_same_v<decltype(static_if<is_column<column_type>{}>(l)(column)), folly::Expected<folly::Unit, sqlite_error>>)
                        return static_if<is_column<column_type>{}>(l)(column);
                    else
                        return static_if<is_column<column_type>{}>(l)(column),folly::makeExpected<sqlite_error>(folly::unit);
                });
            }
        ...
        struct storage_t : storage_base {
          ...
          public:
            ...
            */
             ...
             */
            template<class O, class... Ids>
            folly::Expected<O,sqlite_error> get(Ids... ids) {
                this->assert_mapped_type<O>();
                return this->prepare(sqlite_orm::get<O>(std::forward<Ids>(ids)...)).then(
                    [this](auto& statement)
                    {
                        return this->execute(statement);
                    }
             ...
             */
            template<class O>
            folly::Expected<folly::Unit,sqlite_error> replace(const O &o) {
                this->assert_mapped_type<O>();
                return this->prepare(sqlite_orm::replace(std::ref(o))).then(
                    [this](auto& statement) {return this->execute(statement); }
                );
            }
          ...
          public:
            ...
            template<class T, class... Args>
            folly::Expected<prepared_statement_t<get_all_t<T, Args...>>,sqlite_error> prepare(get_all_t<T, Args...> get_) {
                auto con = this->get_connection();
                sqlite3_stmt *stmt;
                auto db = con.get();
                using context_t = serializator_context<impl_type>;
                ...
                if(sqlite3_prepare_v2(db, query.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
                    ...
                    return sqlite_error(std::error_code(sqlite3_errcode(db), get_sqlite_error_category()),
                                            sqlite3_errmsg(db));
                }
            }
            ...
            template<class T, class... Ids>
            folly::Expected<prepared_statement_t<get_t<T, Ids...>>,sqlite_error> prepare(get_t<T, Ids...> get_) {
                auto con = this->get_connection();
                sqlite3_stmt *stmt;
                auto db = con.get();
                using context_t = serializator_context<impl_type>;
                ...
                auto query = serialize(get_, context);
                if(sqlite3_prepare_v2(db, query.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
                    return { folly::in_place, std::move(get_), stmt, con };
                } else {
                    return folly::makeUnexpected(sqlite_error(std::error_code(sqlite3_errcode(db), get_sqlite_error_category()),
                                            sqlite3_errmsg(db)));
                }
            }
            ...
            template<class T>
            folly::Expected<prepared_statement_t<replace_t<T>>,sqlite_error> prepare(replace_t<T> rep) {
                auto con = this->get_connection();
                sqlite3_stmt *stmt;
                ...
                if(sqlite3_prepare_v2(db, query.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
                    return {folly::in_place,std::move(rep), stmt, con};
                } else {
                    return folly::makeUnexpected(sqlite_error(std::error_code(sqlite3_errcode(db), get_sqlite_error_category()),
                                            sqlite3_errmsg(db)));
                }
            ...
            template<class T>
            folly::Expected<folly::Unit, sqlite_error> execute(const prepared_statement_t<replace_t<T>>& statement) {
                using statement_type = typename std::decay<decltype(statement)>::type;
                using expression_type = typename statement_type::expression_type;
                using object_type = typename expression_object_type<expression_type>::type;
                auto con = this->get_connection();
                ...
                auto &tImpl = this->get_impl<object_type>();
                sqlite3_reset(stmt);
                return tImpl.table.for_each_column([&o, &index, &stmt, db](auto &c)->folly::Expected<folly::Unit, sqlite_error> {
                    using column_type = typename std::decay<decltype(c)>::type;
                    using field_type = typename column_type::field_type;
                    if(c.member_pointer) {
                        if(SQLITE_OK != statement_binder<field_type>().bind(stmt, index++, o.*c.member_pointer)) {
                    ...
                    return folly::makeExpected<sqlite_error>(folly::unit);
                        ...
                        [&](folly::Unit) ->folly::Expected<folly::Unit, sqlite_error> {
                            if (sqlite3_step(stmt) == SQLITE_DONE) {
                                return folly::makeExpected<sqlite_error>(folly::unit);
                            }
                            else {
            ...
            template<class T, class... Ids>
            folly::Expected<T,sqlite_error> execute(const prepared_statement_t<get_t<T, Ids...>> &statement) {
                auto &tImpl = this->get_impl<T>();
                auto con = this->get_connection();
                auto db = con.get();
                auto stmt = statement.stmt;
                ...
                switch(stepRes) {
                    case SQLITE_ROW: {
                        ...
                        object_from_column_builder<T> builder{res, stmt};
                        tImpl.table.for_each_column(builder);
                        return folly::makeExpected<sqlite_error>(std::move(res));
                    } break;
                    case SQLITE_DONE: {
                        return folly::makeUnexpected(sqlite_error(std::make_error_code(sqlite_orm::orm_error_code::not_found)));
                    } break;
                    default: {
                        return folly::makeUnexpected(sqlite_error(std::error_code(sqlite3_errcode(db), get_sqlite_error_category()),
                                                sqlite3_errmsg(db)));
