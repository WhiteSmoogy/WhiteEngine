...
public:
    ...
    thread_specific_ptr(destructor_func func)
       : Destructor(func), TlsIndex(TlsAlloc())
    {
    ...
    ~thread_specific_ptr()
    {
        auto value = TlsGetValue(TlsIndex);
...
int pthread_key_create(pthread_key_t* key, void (*destructor)(void*)) {
  try {
      auto newKey = new thread_specific_ptr<void>(destructor);
    *key = newKey;
    return 0;
...
int pthread_key_delete(pthread_key_t key) {
  try {
    auto realKey = reinterpret_cast<thread_specific_ptr<void>*>(key);
    delete realKey;
    return 0;
  } catch (std::system_error) {
    return -1;
