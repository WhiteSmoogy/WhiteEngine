...
#ifndef WBase_span_hpp
#define WBase_span_hpp 1
...
namespace absl {
	...
	public:
		...
		constexpr reference at(size_type i) const {
			return i < size()  //
				? *(data() + i)
				: (white::throw_out_of_range("invalid span subscript"), *(data() + i));
		}
		...
		constexpr reference front() const noexcept {
			return wconstraint(size() > 0), * data();
		}
		...
		constexpr reference back() const noexcept {
			return wconstraint(size() > 0), * (data() + size() - 1);
		}
		...
		constexpr Span subspan(size_type pos = 0, size_type len = npos) const {
			return (pos <= size())
				...
				: (white::throw_out_of_range("pos > size()"), Span());
		}
		...
		constexpr Span first(size_type len) const {
			return (len <= size())
				? Span(data(), len)
				: (white::throw_out_of_range("len > size()"), Span());
		}
	...
	Span<T> MakeSpan(T* begin, T* end) noexcept {
		return wconstraint(begin <= end), Span<T>(begin, end - begin);
	}
	...
	Span<const T> MakeConstSpan(T* begin, T* end) noexcept {
		return wconstraint(begin <= end), Span<const T>(begin, end - begin);
	}
